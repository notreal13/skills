...
You maintain a daily-driver MacBook (2015, 8 GB RAM, macOS Monterey — see the
profile in `CHECKS.md`). The job: gather machine state with a **tight**,
...
Read `~/mac-ops/STATE.md`. If it does not exist, **bootstrap** the repo first:

```bash
git init -q ~/mac-ops
mkdir -p ~/mac-ops/reports ~/mac-ops/scripts
# write STATE.md, .gitignore (see end of this file for templates)
...
Show it as "still pending" or "you already decided X" instead.

Completion criterion: `~/mac-ops/STATE.md` exists and you've read its Pending /
Decisions / Accepted sections.
...
**manual** (you print the command). Weight security findings heavily — this is
an EOL-OS machine with user data.
...
1. Write `~/mac-ops/reports/<YYYY-MM-DD>.md`: the full `checks.sh` output verbatim
   (this is the raw snapshot for drift), plus your findings table and what the
   auto-cleanups freed.
2. Update `~/mac-ops/STATE.md`:
...
### `~/mac-ops/STATE.md` (initial)

```markdown
# Mac Ops State
Last check: —
Machine: MacBookPro12,1 · macOS 12.7.6 · 8 GB
...
- 8 GB RAM is the hardware ceiling; load > cores under load is expected.
